...
export async function getDashboardKPIs(inicioStr: string, fimStr: string, inicioAntStr: string, fimAntStr: string) {
  ...
  const slotsTotal = Math.max(1, salas * dias * 14)
...
// TODO(performance): reintroduzir cache com tenantId como chave em fase futura quando tráfego justificar.
// unstable_cache foi removido porque não incluía tenantId na chave — causaria vazamento de dados entre tenants.
export async function getDashboardCharts(inicioStr: string, fimStr: string) {
  const db = getTenantDb()
  ...
  const diasInterval = eachDayOfInterval({ start: inicio, end: fim }).slice(0, 31)
  const ocupacaoDias = await Promise.all(
    diasInterval.map(async (dia) => {
...
// TODO(performance): reintroduzir cache com tenantId como chave em fase futura quando tráfego justificar.
// unstable_cache foi removido porque não incluía tenantId na chave — causaria vazamento de dados entre tenants.
export async function getDashboardListas() {
  const db = getTenantDb()
